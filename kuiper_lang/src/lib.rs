...
impl CompileError {
    ...
    pub fn message(&self) -> String {
        match self {
            ...
            CompileError::TypeError(type_error) => type_error.to_string(),
        }
    }
}
...
#[cfg(test)]
pub(crate) fn compile_expression_test(
    data: &str,
    known_inputs: &[&str],
    ...
    compile_expression_with_config(
        data,
        known_inputs,
        &CompilerConfig::default().run_type_checker(true),
    )
}
...
mod tests {
    ...
    use crate::{
        compile_expression_test, compiler::BuildError, format_expression, lex::Token, CompileError,
        TransformError,
    };

    fn compile_err(data: &str, inputs: &[&str]) -> CompileError {
        match compile_expression_test(data, inputs) {
