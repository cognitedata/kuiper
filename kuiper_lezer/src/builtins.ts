...
export const builtIns: KuiperInput[] = [
    ...
    { label: "contains", description: "`contains(x, a)`: Returns `true` if the array or string `x` contains item `a`." },
    ...
    { label: "ends_with", description: "`ends_with(item, substring)`: Returns `true` if `item` ends with `substring`." },
    ...
    { label: "regex_all_matches", description: "`regex_all_matches(haystack, regex)`: Return an array of all the substrings that match the regex. If no match is found, this returns an empty array. Prefer [regex_first_match](#regex_first_match) if all you need is the first match." },
    ...
    { label: "regex_replace", description: "`regex_replace(haystack, regex, replace)`: Replace the first occurence of the regex in the haystack. The replace object supports referencing capture groups using either the index (`$1`) or the name (`$group`). Use `$$` if you need a literal `$` symbol. `${group}` is equivalent to `$group` but lets you specify the group name exactly." },
    { label: "regex_replace_all", description: "`regex_replace_all(haystack, regex, replace)`: Replace each occurence of the regex in the haystack. See [regex_replace](#regex_replace) for details." },
    ...
    { label: "starts_with", description: "`starts_with(item, substring)`: Returns `true` if `item` starts with `substring`." },
